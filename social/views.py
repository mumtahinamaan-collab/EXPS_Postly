...
def serialize_post(post, current_user=None):
    ...
    return {
        ...
        "likes_count": getattr(post, "likes_count", post.likes.count()),
...
def discover_users(request):
    ...
    search = request.data.get("input", "").strip()
...
def toggle_follow(request):
    ...
    already_following = user.following.filter(
        id=target_user.id
    ).exists()
...
def user_social_data(request, user_id):
    ...
    return Response({
        ...
        "posts": [
            serialize_post(post, request.user)
            for post in posts
        ],
...
def post_feed(request):
    ...
    # User's own posts + posts from users they follow
...
def toggle_like(request, post_id):
    ...
    already_liked = post.likes.filter(
        id=user.id
    ).exists()
...
def get_chat_messages(request):
    ...
    try:
        other_user = User.objects.get(id=to_user_id)
        ...
        "messages": [
            {
                ...
                "to_user_id": str(message.to_user.id),
                "text": message.text,
